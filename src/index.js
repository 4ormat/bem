...
const bem = (blockName, elementName, modifierName) => {
  ...
  return {
    toString: function() {
      ...
      const modifierObject = isObject(modifierName)
        ? modifierName
        : { [modifierName]: true };

      const modifierArray = isArray(modifierObject)
        ? modifierObject
        : keys(pickBy(modifierObject, key => !!key));
      ...
      return (
        classes +
        " " +
        modifierStrings.map(modifier => `${classes}--${modifier}`).join(" ")
      );
    },
    toSelector: function() {
